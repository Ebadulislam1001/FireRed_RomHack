...
#define SPORE_MOVES_END 0xFFFF
#define PUNCH_MOVES_END 0xFFFF
#define KICK_MOVES_END 0xFFFF
...
static const u16 sSporeMovesTable[] =
{
    MOVE_LEECH_SEED,
    MOVE_POISON_POWDER,
    MOVE_STUN_SPORE,
...
{
    MOVE_POUND, // DRAIN_PUNCH
    MOVE_COMET_PUNCH, // BULLET_PUNCH
    MOVE_MEGA_PUNCH,
    MOVE_FIRE_PUNCH,
    MOVE_ICE_PUNCH,
    MOVE_THUNDER_PUNCH,
    MOVE_SONIC_BOOM, // RAGE_FIST
    MOVE_DIZZY_PUNCH,
    MOVE_MACH_PUNCH,
    MOVE_SCARY_FACE, // POWER_UP_HIT
    MOVE_DYNAMIC_PUNCH,
    MOVE_FOCUS_PUNCH,
    MOVE_METEOR_MASH,
    MOVE_SHADOW_PUNCH,
...
static const u16 sKickMovesTable[] =
{
    MOVE_DOUBLE_KICK,
    MOVE_MEGA_KICK,
    MOVE_JUMP_KICK,
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u8 ability, u8 special, u16 moveArg)
{
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI)) // Why isn't that check done at the beginning?
    {
        ...
        if (IS_BATTLE_TYPE_GHOST_WITHOUT_SCOPE(gBattleTypeFlags)
         && (gLastUsedAbility == ABILITY_INTIMIDATE || gLastUsedAbility == ABILITY_ILLUMINATE || gLastUsedAbility == ABILITY_TRACE))
            return effect;
        ...
        case ABILITYEFFECT_ENDTURN: // 1
            if (gBattleMons[battler].hp != 0)
            {
                ...
                case ABILITY_SOLAR_POWER:
                    ...
                    break;
                case ABILITY_SENJUTSU:
                    if (gBattleMons[battler].maxHP > gBattleMons[battler].hp)
                    {
                        gLastUsedAbility = ABILITY_SENJUTSU; // why
                        BattleScriptPushCursorAndCallback(BattleScript_SenjutsuActivates);
                        gBattleMoveDamage = gBattleMons[battler].maxHP / 16;
                        if (gBattleMoveDamage == 0)
                            gBattleMoveDamage = 1;
                        gBattleMoveDamage *= -1;
        ...
        case ABILITYEFFECT_ON_DAMAGE: // Think contact abilities.
            ...
            case ABILITY_CUTE_CHARM:
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 ...
                 && (Random() % 100) < 30 // 30% chance to inflict attraction
        ...
        case ABILITYEFFECT_INTIMIDATE1: // 9
            ...
            break;
        case ABILITYEFFECT_ILLUMINATE1: // 20
            for (i = 0; i < gBattlersCount; i++)
            {
        ...
        case ABILITYEFFECT_INTIMIDATE2: // 10
            ...
            break;
        case ABILITYEFFECT_ILLUMINATE2: // 21
            for (i = 0; i < gBattlersCount; i++)
            {
                if (gBattleMons[i].ability == ABILITY_ILLUMINATE && (gStatuses3[i] & STATUS3_ILLUMINATE_POKES))
                {
...
u8 IsMonDisobedient(void)
{
    ...
    else
    {
        ...
        calc = (Random() & 255);
        if (calc < obedienceLevel && !(gBattleMons[gBattlerAttacker].status1 & STATUS1_ANY) && gBattleMons[gBattlerAttacker].ability != ABILITY_INSOMNIA)
        {
            // try putting asleep
            int i;
            for (i = 0; i < gBattlersCount; i++)
